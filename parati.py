...
def escribir_mensaje_con_brillo():
    mensaje = "En tan poco tiempo, te volviste alguien tan especial\nque cada día quiero cuidarte más y más"
    
    # Efecto de brillo
    for _ in range(3):
        t.penup()
        t.goto(0, 289)
        # Primero dibujamos un contorno dorado para el efecto de brillo
        t.color("#a0a0a0")  # sombra ploma
        t.write(mensaje, align="center", font=("Arial", 16, "bold"))
        time.sleep(0.1)
        
        # Luego escribimos el texto en negro encima
        t.goto(0, 287)
        t.color("black")
        t.write(mensaje, align="center", font=("Arial", 16, "bold"))
        time.sleep(0.1)
...
def dibujar_A():
    t.penup()
    t.goto(-20, 50)  # Ajustado posición para centrar mejor
    t.color("black")  # Cambiado a negro
    t.pendown()
    t.pensize(12)  # Reducido el grosor
    
    # Dibujar la letra A
    t.seth(-70)
    t.forward(80)  # Reducido tamaño
    t.backward(80)
    t.seth(-110)
    t.forward(80)
    t.backward(40)
...
escribir_mensaje_con_brillo()

# Restauramos el grosor del trazo para los corazones
t.pensize(5)
...
# Dibujamos la A después de los corazones
dibujar_A()
